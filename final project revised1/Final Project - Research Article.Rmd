---
title: "Final Project-Research Article"
...
horse %>%
  ...
  ggplot(., aes(x=age,fill=outcome,y = n)) + 
    ...
    geom_text(aes(label = outcome), size = 3, position=position_dodge(width=0.9), vjust=-0.25)+
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
require(gridExtra)
ggplot(data=horse,aes(y=rectal_temp,fill=outcome)) + geom_boxplot()

ht1= ggplot(data=horse %>% filter(outcome=="died"),aes(y=rectal_temp)) + geom_histogram(bins=10,color="red") + coord_flip() + labs(title = "Died Horses by rectal temp") + theme(plot.title = element_text(size=10))

ht2= ggplot(data=horse %>% filter(outcome=="euthanized"),aes(y=rectal_temp)) + geom_histogram(bins=10,color="green") + coord_flip() + labs(title = "Euthan. Horses by rectal temp")+ theme(plot.title = element_text(size=10))
ht3= ggplot(data=horse %>% filter(outcome=="lived"),aes(y=rectal_temp)) + geom_histogram(bins=10,color="blue") + coord_flip() + labs(title = "Lived Horses by rectal temp")+ theme(plot.title = element_text(size=10))

ggplot(data=horse,aes(y=rectal_temp,fill=outcome)) + geom_histogram(bins=10,color="blue") + coord_flip() + labs(title = "Lived Horses by rectal temp")


grid.arrange(ht1,ht2,ht3,ncol=3)
...
grid.arrange(bg1,bg2 + scale_fill_manual(values=c("#56B4E9", "#E7298A")),ncol=2)
grid.arrange(bg3 + scale_fill_manual(values=c("#56B4E9", "#E7298A")),ncol=2)
