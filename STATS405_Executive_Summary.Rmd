...
```{r echo = F, warning = F,  fig.show="hold", message = F , fig.width=8,fig.height = 6}
#, fig.width=5, fig.height = 4, out.width = ".7\\textwidth",
#, fig.align="right", wrapfigure = list("R", .6)
#, fig.cap = "Horse Outcome vs Surgery Performed"
horse %>%
  group_by(surgical_lesion, outcome) %>%
  summarise(n = n()) %>%
  ggplot(., aes(x=surgical_lesion,fill=outcome,y = n)) + 
  geom_bar(stat = "identity",position=position_dodge()) +
  labs(x = "Indicator if Surgery was Performed", y = "Count of Horses", fill = "Outcome", title = "Horse Outcome vs Surgery Performed") +
  scale_fill_manual(values=safe_colorblind_palette[1:3]) +
    geom_text(aes(label = n), size = 3.5, position=position_dodge(width=1), vjust=-0.25) +
  theme(aspect.ratio = 0.5, axis.title.x = element_text(size = 8), axis.title.y = element_text(size = 8), plot.title = element_text(hjust = 0.5, size = 11))
```
